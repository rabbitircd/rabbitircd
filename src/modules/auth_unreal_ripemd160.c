/*
 * RabbitIRCd, src/modules/auth_unreal_ripemd160.c
...
ModuleHeader MOD_HEADER(auth_unreal_ripemd160)
  = {
        "auth_unreal_ripemd160",
        "$Id$",
        "ripemd160 authentication type",
        "3.2-b8-1",
        NULL 
    };
...
*/
int unreal_ripemd160_validate(aClient *cptr, anAuthStruct *as, const char *para)
{
char buf[512];
int i, r;
...
const char *unreal_ripemd160_make_hash(const char *para)
{
static char buf[128];
...
static struct auth_ops unreal_ripemd160_ops = {
	.name = "ripemd160",
	.validate = unreal_ripemd160_validate,
	.make_hash = unreal_ripemd160_make_hash,
};

/* sure, this is ugly, but it's our fault. -- Syzop */
static struct auth_ops unreal_ripemd_160_ops = {
	.name = "ripemd-160",
	.validate = unreal_ripemd160_validate,
	.make_hash = unreal_ripemd160_make_hash,
};
...
DLLFUNC int MOD_INIT(auth_unreal_ripemd160)(ModuleInfo *modinfo)
{
...
DLLFUNC int MOD_LOAD(auth_unreal_ripemd160)(int module_load)
{
	auth_register_ops(&unreal_ripemd160_ops);
	auth_register_ops(&unreal_ripemd_160_ops);

	return MOD_SUCCESS;
}

/* Called when module is unloaded */
DLLFUNC int MOD_UNLOAD(auth_unreal_ripemd160)(int module_unload)
{
	auth_unregister_ops(&unreal_ripemd160_ops);
	auth_unregister_ops(&unreal_ripemd_160_ops);
